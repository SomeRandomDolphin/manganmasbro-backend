...
export const queryRecipebyID = async (idInput: number) => {
  ...
  const ingredientsArray = Array.from(
    { length: 20 },
    (_, i) => recipe.recipeFromIngredient[`ingredient${i + 1}`],
  ).filter((ingredient) => ingredient !== null && ingredient !== undefined);
  const measuresArray = Array.from(
    { length: 20 },
    (_, i) => recipe.recipeFromMeasure[`measure${i + 1}`],
  ).filter((measure) => measure !== null && measure !== undefined);
  const stepsArray = Array.from(
    { length: 20 },
    (_, i) => recipe.recipeFromStep[`step${i + 1}`],
